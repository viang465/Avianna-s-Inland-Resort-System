...
function getMailer(): PHPMailer {
    ...
    $mail->Password   = 'dmhkacwoqpejzxyy'; // 16-char App Password
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($email) || empty($contact) || empty($address) || empty($checkin) || empty($checkout) || empty($pax) || empty($payment)) {
        ...
        $stmt = $conn->prepare("
            ...
            (name, email, contact, address, room_type, cottage_type, pax, checkin, checkout, payment_method, total_price, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <p class="text-center text-muted mb-4" style="font-size:.9rem;">
                A confirmation email will be sent immediately. Approval within 24 hours.
            </p>
            ...
            <form method="POST" class="row g-3" novalidate id="bookingForm">
                ...
                <div class="col-12"><hr class="section-divider"><p class="section-label">🏨 Stay Details</p></div>
